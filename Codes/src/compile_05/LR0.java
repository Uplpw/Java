...
public class LR0 {
			...
			if(map.get(i).size()==1) {
				if(map.get(i).get(0).getNewRight().substring(map.get(i).get(0).getNewRight().length()-1, map.get(i).get(0).getNewRight().length()).equals(".")) {
					if(map.get(i).get(0).getNewLeft().getSignal().equals(grammer.getSignalVn().getSignal())) {
						//接受
						table[i*line+grammer.getListSignalVt().size()]="acc";
					}else {
						//归约
	...
	public boolean process(String[] mm, String str, Grammer grammer) {
		...
		stackS.push("0");
		stackC.push("#");
		//System.out.print((step++)+"\t\t"+stackS.get(stackS.size()-1)+"\t\t"+stackC.get(stackC.size()-1)+"\t\t"+str);
		int count=0;
		...
		while(flag) {
			...
			for(int i=0;i<stackC.size();i++) {
				System.out.print(stackC.get(i));
			}
			System.out.print("\t\t"+str.substring(count, str.length())+"\t\t");
			...
			if(string.equals("")) {
				...
				if(string.substring(0, 1).equals("s")) {
					System.out.println("["+stackS.get(stackS.size()-1)+", "+str.substring(count, count+1)+"]="+string+", 移进"+
							str.substring(count, count+1)+", 状态"+stackS.get(stackS.size()-1)+"入栈");
					stackS.push(string.substring(1, string.length()));
					stackC.push(str.substring(count, count+1));
					count++;
				}else if(string.substring(0, 1).equals("r")) {
					System.out.println("["+stackS.get(stackS.size()-1)+", "+str.substring(count, count+1)+"]="+string+", 归约");
					
					ArrayList<String> list=Tools.enCode(grammer);
					int length=list.get(Integer.parseInt(string.substring(1, string.length()))).length()-1;
					...
					stackC.push(list.get(Integer.parseInt(string.substring(1, string.length()))).substring(0, 1));
					//System.out.println("2rc: "+stackC.get(stackC.size()-1));
					
					int indexVn=Tools.isContainVn(grammer.getListSignalVn(), 
							list.get(Integer.parseInt(string.substring(1, string.length()))).substring(0, 1));
